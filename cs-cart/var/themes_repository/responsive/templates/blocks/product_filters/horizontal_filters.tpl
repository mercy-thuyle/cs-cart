...
{foreach from=$items item="filter" name="filters"}
    ...
    <div class="ty-horizontal-product-filters-dropdown">
        <div id="sw_elm_filter_{$filter.filter_id}" class="ty-horizontal-product-filters-dropdown__wrapper cm-combination {if $filter.selected_variants || $filter.selected_range}active{/if}">{$filter.filter}{if $filter.selected_variants} ({$filter.selected_variants|sizeof}){/if}{if $reset_url}<a class="cm-ajax cm-ajax-full-render cm-history" href="{$reset_url|fn_url}" data-ca-event="ce.filtersinit" data-ca-target-id="{$ajax_div_ids}">{include_ext file="common/icon.tpl" class="ty-icon-cancel-circle"}</a>{/if}{include_ext file="common/icon.tpl" class="ty-horizontal-product-filters-dropdown__icon ty-icon-down-micro"}</div>
