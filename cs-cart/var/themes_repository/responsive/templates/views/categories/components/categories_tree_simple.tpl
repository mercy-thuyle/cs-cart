...
<tr>
    ...
    <th class="ty-categories-picker__title" style="width: 97%">
        ...
        <div class="ty-float-left">
            {$icon_down_dir = "ty-icon-down-dir ty-dir-list cm-combinations{if $expand_all} hidden{/if}"}
            {$icon_right_dir = "ty-icon-right-dir ty-dir-list cm-combinations{if !$expand_all} hidden{/if}"}
            ...
            <span style="padding-{$direction}: {$_shift}px;">
                {if $cur_cat.has_children || $cur_cat.subcategories}
                    ...
                    <span id="on_{$comb_id}" class="cm-combination {if isset($path.$cat_id) || $expand_all}hidden{/if}">
                        {include_ext file="common/icon.tpl"
                            class="ty-dir-list ty-icon-right-dir"
                    ...
                    <span id="off_{$comb_id}" class="cm-combination {if !isset($path.$cat_id) && (!$expand_all || !$show_all)}hidden{/if}">
                        {include_ext file="common/icon.tpl"
                            class="ty-dir-list ty-icon-down-dir"
