{if $image.location == "wishlist"}
    ...
    {if $delete_link}<a class="cm-ajax" href="{$delete_link|fn_url}">{/if}{if !($po.required == "Y" && $images|count == 1)}{include_ext file="common/icon.tpl"
        class="ty-icon-cancel-circle ty-hand"
    ...
    }{/if}{if $delete_link}</a>{/if}<span class="upload-filename">{if $download_link}<a href="{$download_link}">{/if}{$image.name}{if $download_link}</a>{/if}</span>
{/if}
