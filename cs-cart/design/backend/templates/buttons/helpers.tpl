...
{function name="btn" text="" href="" title="" onclick="" target="" class="" data=[] form="" method="" raw=false}
    ...
    {if $type === "text" || $type === "button"}
        ...
        >
        {if $icon && $icon_first}{include_ext file="common/icon.tpl" class=$icon}{/if}
            {if $raw == false}
                {$text}
            {else}
                {$text nofilter}
    ...
    {if $type == "dialog"}
        ...
        {btn type="text" text=$text class="cm-dialog-opener `$class`" href=$href id=$id title=$title data=$data method=$method}
    {/if}
