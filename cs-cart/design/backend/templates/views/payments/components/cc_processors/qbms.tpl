...
{if $processor_params.oauth_client_id && $processor_params.oauth_client_secret}
    ...
    {__("payments.qbms.tip_fill_redirect", ["[url]" => "{"current"|fn_payment_url:"qbms.php"}?qb_action=auth_callback&payment_id={$payment_id}"]) nofilter}
{else}
    {__("payments.qbms.configure_payment_method", ["[product]" => $smarty.const.PRODUCT_NAME]) nofilter}
{/if}
