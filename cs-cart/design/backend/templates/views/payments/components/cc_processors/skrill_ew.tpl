{$register_url = "https://signup.skrill.com/business/"}
{$pay_to_email = $processor_params.pay_to_email|escape:url}

<p>{__("text_skrill_notice_register", ["[register_url]" => $register_url]) nofilter}</p>
<hr>
...
<div class="control-group">
    ...
    <div class="controls">
        <input type="text" name="payment_data[processor_params][pay_to_email]" id="pay_to_email_{$payment_id}" value="{$processor_params.pay_to_email}"  size="60">
    </div>
</div>
...
<p>
    <a onclick="Tygh.$.ceAjax('request', '{"payment_notification.activate?payment=skrill_ewallet&payment_id=`$payment_id`&email="|fn_url nofilter}' + Tygh.$('#pay_to_email_{$payment_id}').val() + '&cust_id=' + Tygh.$('#customer_id_{$payment_id}').val() + '&platform=21477207' + '&merchant_firstname=' + Tygh.$('#merchant_firstname_{$payment_id}').val() + '&merchant_lastname=' + Tygh.$('#merchant_lastname_{$payment_id}').val(), {ldelim}method: 'GET'{rdelim}); return false;" href="{"payment_notification.activate?payment=skrill_ewallet&payment_id=`$payment_id`&email=`$pay_to_email`&cust_id=`$processor_params.customer_id`&platform=21477207"|fn_url}">{__("activate_skrill_merchant_tools")}</a>
</p>
<p>
...
<div class="control-group">
    ...
    <div class="controls">
        ...
        <p class="muted description">{__("text_skrill_currs_notice", ["[link]" => $cur_man]) nofilter}</p>
    </div>
</div>
...
<p>{__("text_skrill_support") nofilter}</p>