...
{else}
{$can_be_disabled = $can_be_disabled|default:true}
<div class="control-group">
    <label class="control-label cm-required">{__("status")}:</label>
    <div class="controls">
        {if $items_status}
        ...
        {/if}
            <label class="radio inline" for="{$id}_{$obj_id|default:0}_d"><input type="radio" name="{$input_name}" id="{$id}_{$obj_id|default:0}_d" {if $obj.status === "ObjectStatuses::DISABLED"|enum}checked="checked"{/if} value="{"ObjectStatuses::DISABLED"|enum}" {if !$can_be_disabled}disabled="disabled"{/if}/>{__("disabled")}</label>
        {/if}
    </div>
</div>
{/if}