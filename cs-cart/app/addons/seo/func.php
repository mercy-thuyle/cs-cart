...
function fn_seo_update_tree_object($object_id, $object_type, $params)
{
    ...
    if (!isset($params['after_deletion']) || !$params['after_deletion']) {
        ...
        foreach ($lang_codes as $lang_code) {
            $seo_name = (isset($params['seo_names'][$lang_code]))
                ? $params['seo_names'][$lang_code]
                : fn_seo_get_name($object_type, $object_id, '', $params['company_id'], $lang_code);
...
/**
 * The "ajax_destruct_before_response" hook handler.
 *
 * Actions performed:
 *     - Adding the page number to the page header when an ajax request is made
 *
 * @param \Tygh\Ajax $ajax                Ajax instance
 ...
 * @return void
 */
function fn_seo_ajax_destruct_before_response(Ajax $ajax, $text, $embedded_is_enabled)
{
    ...
    /** @var array<string, string|int> $search */
    $search = $view->getTemplateVars('search');

    /** @var string $title */
    $title = $ajax->getAssignedVar('title');
    $ajax->assign('title', $title . fn_get_seo_page_title($search));
}
...
/**
 * Updates the SEO-name of the imported product after updating the product name.
 *
 ...
 * @return void
 */
function fn_seo_import_product_descr_post(array $data, $product_id, $prod_company_id, $field, $is_new, array $object)
{
    if ($field !== 'product') {
        return;
    }
    ...
    $params['object_types'] = ObjectTypes::PRODUCT;
    $params['seo_names'] = [];

    foreach ($data as $lang_code => $product_name) {
        $old_seo_name = fn_seo_get_name(ObjectTypes::PRODUCT, $product_id, '', $prod_company_id, $lang_code) ;
        $params['seo_names'][$lang_code] = ($old_seo_name === '')
            ? $product_name
            : $old_seo_name;
    }
