...
if ($mode == 'customer_geolocation') {
    ...
    $_product_id = (int) (isset($full_product_id[1]) ? $full_product_id[1] : $_REQUEST['product_id']);

    $location = fn_geo_maps_get_customer_stored_geolocation();
    list($shipping_methods, $shippings_summary) = ShippingEstimator::getShippingEstimation(
        $_product_id,
        $location,
        $auth,
