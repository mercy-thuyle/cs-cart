        ...
        if ($called == false && $this->_internal_request) {
            ...
            if ($clean_raw_response = !empty($this->result_ids)) {
                ...
                if ($this->full_render && preg_match('/<title>(.*?)<\/title>/s', $text, $m)) {
                    ...
                    $title = $view->getTemplateVars('page_title');

                    if (empty($title)) {
                        /** @var array<int, array<string, string>> $breadcrumbs */
                        $breadcrumbs = $view->getTemplateVars('breadcrumbs');

                        /** @var string $language_direction */
                        $language_direction = $view->getTemplateVars('language_direction');

                        /** @var array<string, string> $location_data */
                        $location_data = $view->getTemplateVars('location_data');
                        $suffix = !empty($location_data['title']) ? $location_data['title'] : '';

                        $title = fn_generate_page_title($breadcrumbs, $language_direction, $suffix);
                    }
                }
